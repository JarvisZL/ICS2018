...
void difftest_step(uint32_t eip) {
  ...
  // Set `nemu_state` to `NEMU_ABORT` if they are not the same.
   ...
   printflog("EIP:QEME      NEMU\n");
   printflog("EIP:%-8x  %-8x\n",ref_r.eip,cpu.eip);
   printflog("REGS:eax       ecx       edx       ebx       esp       ebp       esi       edi\n");
   printflog("QEMU:%-8x  %-8x  %-8x  %-8x  %-8x  %-8x  %-8x  %-8x\n",ref_r.eax,ref_r.ecx,ref_r.edx,ref_r.ebx,ref_r.esp,ref_r.ebp,ref_r.esi,ref_r.edi);
   printflog("NEMU:%-8x  %-8x  %-8x  %-8x  %-8x  %-8x  %-8x  %-8x\n",cpu.eax,cpu.ecx,cpu.edx,cpu.ebx,cpu.esp,cpu.ebp,cpu.esi,cpu.edi);
